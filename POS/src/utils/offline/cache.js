...
function getCacheVersion() {
	...
	const storedVersion = Number.parseInt(localStorage.getItem("pos_next_cache_version") || "1");
...
export async function getCachedSalesPersons(posProfile) {
	try {
		const persons = await db.sales_persons.where("pos_profile").equals(posProfile).toArray();
